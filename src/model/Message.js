...
export class Message extends Model
{
    ...
    getViewElement(me = true)
    {
        ...
        // retorna o html de uma div para cada tipo de 
        ...
        switch (this.type) 
        {
            ...
            case 'image':
                ...
                // quando conseguir carregar a imagem, remove a bolinha que fica rodando
                div.querySelector('.message-photo').on('load', e =>
                {
                    // finalizou o envio da imagem
                    console.log('load ok');
                    // oculta a bolinha rodando
                    div.querySelector('._340lu').hide();
                    // mostra a imagem que foi enviada
                    div.querySelector('.message-photo').show();
                    // remove a altura para ficar dinamico conforme a imagem selecionada
        ...
        // define se a mensagem foi recebida ou enviada
        let className = 'message-in';

        // verifica se a mensagem é minha
        // se for, carrega também o status se a mensagem já foi lida ou recebida
        if (me)
        {

            className = 'message-out';

            // procura a div com a classe 'message-time'
            // acessa o item pai, e adiciona um novo filho, que ficará logo abaixo o elemento 'message-time'
            div.querySelector('.message-time').parentElement.appendChild(this.getStatusViewElement());

        }

        // busca o primeiro elemento da div, e inclui a classe
        // que define se a mensagem foi recebida ou enviada
    ...
    static getRef(chatId)
    {
        ...
        // retorna o caminho da mensagens deste usuário
        // firebase > chats > chatId > messages
        return Firebase.db()
                       .collection('chats')
                       .doc(chatId)
                       .collection('messages');
